...
def test_opcoes_clicaveis():
    ...
    sid = d["session_id"]
    d = _chat("2", sid)                                       # tela de info (outorga)
    labels = [o["label"] for o in d["options"]]
    assert "Menu inicial" in labels and "Abrir chamado" in labels
    assert any(o["value"].startswith("faq:") for o in d["options"])   # sub-botoes
    # na revisao do chamado, aparecem Confirmar/Recomecar/Cancelar
